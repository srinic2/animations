#Visual animation of two sets of simple pendulums
#One set starts from left of center and the other from right of center
#See pendulums-5.py for comments
...
def drawPendulum1(canvas,data):
    ...
    for i in range(len(data.pendulums1)):
        ...
        #canvas.create_line(data.centerX, data.centerY, posX, posY, fill = "black", width = 1)
...
def makePendulums2(data):
    ...
    posY = data.margin + length*math.cos(theta)
    omega = 0
    data.pendulums2.append([ posX, posY, length, omega, theta ])
    data.length2 += 8
    data.counter2 += 1
    
def movePendulum2(data):
    for i in range(len(data.pendulums2)):
        dt = 0.1
        
        posX = data.pendulums2[i][0]
        posY = data.pendulums2[i][1]
        length = data.pendulums2[i][2]
        omega = data.pendulums2[i][3]
        theta = (data.pendulums2[i][4])
        ...
        #Update all the values in data.pendulum
        data.pendulums2[i][0] = posX
        data.pendulums2[i][1] = posY
        data.pendulums2[i][3] = omega
        data.pendulums2[i][4] = theta
        
        
def drawPendulum2(canvas,data):
    if data.time % 100 == 0:
        n = random.randint(0, len(data.fill)-1)
        data.color = data.fill[n]
    for i in range(len(data.pendulums2)):
...
def timerFired(data):
    ...
    movePendulum1(data)
    if data.counter2 < 20: makePendulums2(data)
    movePendulum2(data)
        
def redrawAll(canvas, data):
    drawLines1(canvas,data)
    drawLines2(canvas,data)
    drawPendulum1(canvas, data)
    drawPendulum2(canvas, data)
